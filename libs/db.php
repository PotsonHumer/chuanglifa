<?php
	...
	class DB{
		...
		*/
		...
		public static function update($tbl_name,array $input,$primary=false){
			...
			if(is_array($input) && count($input)){
				...
				if(is_array($primary)){
					# 指定目標欄位
					list($last_key) = array_keys($primary);
					$where_handle = $last_key." = '".$primary[$last_key]."'"; # 組建 where 字串
				}else{
					if(empty($primary)){
						...
						$primarySQL = "SHOW KEYS FROM {$tbl_name} WHERE Key_name = 'PRIMARY'";
						$primaryRS = self::execute($primarySQL);
						$keysRow = mysql_fetch_assoc($primaryRS);

						$where_handle = $keysRow["Column_name"]." = '".$input[$keysRow["Column_name"]]."'";
						unset($input[$keysRow["Column_name"]]);
					}
				}
