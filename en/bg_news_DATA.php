...
$id = $_GET["a_id"];
//news選項
$query_title = "SELECT * FROM bg_news_title";
...
$result_4A = mysqli_query($dbc,$query_4A)
	or die ('資料庫連結失敗');
$row = mysqli_fetch_array($result_4A);
...
<form action="bg_news_INSERT.php" method="post">
    <input type="hidden" name="id" class="text_1"/>
	<p>網站抬頭:</p>
    <input type="text" name="web_title"/><br />
    <p>FB預覽圖:</p>
    <input type="text" name="fb_imge"><a class="img_manage" href="#"><input type="button" value="▶ 點擊選擇圖片" /></a><font color="Green" face="微軟正黑體" size="0">建議尺寸:w150 x h150 / 72dpi</font><br />
	<p>FB內容描述:</p>
    <textarea name="fb_ds"/></textarea><br />
	<p>圖片:</p>
    <input type="text" name="image"><a class="img_manage" href="#"><input type="button" value="▶ 點擊選擇圖片" /></a><font color="Green" face="微軟正黑體" size="0">建議尺寸:w200 x h150 / 72dpi</font><br />
	<p>選項名稱:</p>
    <select name="project"/>
    <?php
	...
	<p>標題名稱:</p>
    <input type="text" name="title"  class="title_text"/><font color="Green" face="微軟正黑體" size="0"> ▪ 必填欄位</font></a>
    <p>內容:</p>
